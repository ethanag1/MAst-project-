...
saveas(gcf, '/home/ethan/fig_BOD(p2)/mi_all_channels_significant_uncorrected_BOD(p2).png');
...
% Save figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/mi_channel1_significantlog_uncorrected_BOD(p2).png');
...
hold off;
saveas(gcf, '/home/ethan/fig_BOD(p2)/mi_channel1_significantnormal_uncorrected_BOD(p2).png');
disp(max(time_ms));
...
% Save the figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/within_channel_coi_significant_channels_uncorrected_BOD(p2).png');
...
% Save the figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/within_channel_signifcoi_significant_channels_uncorrected_BOD(p2).png');
disp('✅ Saved COI plot for all significant channels.');

%% Generate and save separate COI plots for each significant channel

% Define output directory for saving figures
output_dir = '/home/ethan/fig_BOD(p2)/co-I for significant channels uncorrected method';  
...
for i = 1:length(significant_channels)
    ...
    filename = fullfile(output_dir, ['within_channel_coi_channel_' num2str(ch) '_uncorrected_BOD(p2).png']);
