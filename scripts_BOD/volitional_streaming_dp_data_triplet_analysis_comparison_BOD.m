...
BaselineMatrix_avrg = mean(BaselineMatrix_3D, 3);

% Ensure the result is truly 2D by removing the singleton dimension
BaselineMatrix_avrg = squeeze(BaselineMatrix_avrg);

% Display confirmation
disp('✅ Successfully collapsed the 3D matrix into a true 2D matrix by averaging.');
disp(['Size of new 2D matrix: ', num2str(size(BaselineMatrix_avrg,1)), ' x ', num2str(size(BaselineMatrix_avrg,2))]);
...
columnData = BaselineMatrix_avrg(:, columnIndex);
...
numChannels = size(BaselineMatrix_avrg, 2);
...
x = 1:size(BaselineMatrix_avrg, 1);
time = x/srate*1000;
% Create figure
figure;
...
for i = 1:numChannels
    ...
    plot(time, BaselineMatrix_avrg(:, i), 'b', 'LineWidth', 1); % Plot channel in blue
    ...
    yLimit = max(abs(BaselineMatrix_avrg(:))); % Get max absolute value for symmetry
    ylim([-yLimit, yLimit]); % Make y-axis symmetric around 0
    
    % Add smaller channel number label in the top-right corner
    text(max(time) * 0.95, max(BaselineMatrix_avrg(:)) * 0.9, num2str(i), ...
...
dp_data_ep = permute(BaselineMatrix_3D, [2 1 3]);
...
% Plot the selected column
figure; % Create a new figure window
stdshade_acj(squeeze(dp_data_ep(columnIndex,:,:))', 0.2, 'b', time);
...
% Save the figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/triplet_comparison_test_for_single_channel_BOD(p2).png');
...
subplot(10,20,chan)
 stdshade_acj(squeeze(dp_data_ep(chan,:,:))',0.2,'b')

end

saveas(gcf, '/home/ethan/fig_BOD(p2)/triplet_comparison_test_for_all_channels_BOD(p2).png');
...
for i = 1:numChannels
    subplot(gridRows, gridCols, i); % Arrange in a grid
        stdshade_acj(squeeze(dp_data_ep(i,:,:))',0.2,'b')
    ...
    text(max(time) * 1, max(BaselineMatrix_avrg(:)) * 0.9, num2str(i), ...
