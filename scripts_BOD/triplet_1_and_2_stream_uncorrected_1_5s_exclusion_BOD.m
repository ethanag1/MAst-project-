...
Percentage_triplets_identified = (length(selectedIndices_stimOne) + length(selectedIndices_stimTwo)) / 500 * 100;
disp('Percentage of triplets retained using exclusion method');
...
title(['Baseline Corrected and 1.5s exclusion zone around button presses Plot of Channel ', num2str(columnIndex)]); 
...
% Save the figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/baseline_corrected_separated_triplets_1_5s_exclusion_for_channel29_BOD(p2).png');
...
% Add super title
sgtitle('1 stream and 2 stream Baseline Corrected Triplets with 1.5s exclusion zone for All Channels');

% Save the figure
saveas(gcf, '/home/ethan/fig_BOD(p2)/baseline_corrected_separated_triplets_1.5s_exclusion_for_all_channels_BOD(p2).png');
