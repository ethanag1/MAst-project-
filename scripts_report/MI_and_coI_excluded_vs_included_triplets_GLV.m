...
combinedIndices_excluded = sort([selectedIndices_stimOne; selectedIndices_stimTwo]);


%% finding the indices where perception is 1 stream or 2 streams, keeping only values within 1.5s of each button press

% Defining the inclusion window in terms of index length
inclusionWindow = 1.5 * srate; % 1.5 seconds

% Define button presses as changes in perception
button_presses = find(diff(perception) ~= 0) + 1;

% Generate a logical mask of all timepoints within 1.5s of any button press
include_mask = false(size(perception)); % same length as perception

for i = 1:length(button_presses)
    start_idx = max(1, round(button_presses(i) - inclusionWindow));
    end_idx = min(length(perception), round(button_presses(i) + inclusionWindow));
    include_mask(start_idx:end_idx) = true;
end

% Find all indices for 1 and 2 that are within the inclusion mask
indices_1 = find(perception == 1 & include_mask);
indices_2 = find(perception == 2 & include_mask);
...
for i = 1:length(block_starts_1)
    block = indices_1(block_starts_1(i):block_ends_1(i));
    filtered_indices_1 = [filtered_indices_1, block]; % Keep full block
end
...
for i = 1:length(block_starts_2)
    block = indices_2(block_starts_2(i):block_ends_2(i));
    filtered_indices_2 = [filtered_indices_2, block]; % Keep full block
end
...
% Combine and sort the indices
combinedIndices_included = sort([selectedIndices_stimOne; selectedIndices_stimTwo]);

% combine the two sets of indices
selectedIndices = sort([combinedIndices_excluded; combinedIndices_included]);
...
l_ = zeros(size(selectedIndices));

% Label trials: 0 for stimOne, 1 for stimTwo
l_(ismember(selectedIndices, combinedIndices_excluded)) = 0;
l_(ismember(selectedIndices, combinedIndices_included)) = 1;
...
%% calculating the mean coi across all significant channels 
...
[FFIall, FFi_mean, FFi_r, FFi_s, FFm_sum, FFmr_sum, FFms_sum, MI1_out, MI2_out] = get_plotting_CoI_simple(acc);
...
yticks_MI = [0, round(max(MI2_out(:))+0.25*max(MI2_out(:)), 3, 'significant')/2 ];
ylimits_CoI = [-50 550];        % Adjust based on data range
ylimit_MI = [round(min(MI2_out(:)) + 0.1 * min(MI2(:)), 3, 'significant'), round(max(MI2(:)) + 0.25 * max(MI2(:)), 3, 'significant')/2];  % Adjust based on MI data range
y_labels_CoI = [0, 275, 550];
y_labels_MI = [0, round(max(MI2_out(:)) + 0.25 * max(MI2_out(:)), 3, 'significant')/2];
...
% Add super title
sgtitle('GLV', 'FontSize', 25 );

% Save the figure
saveas(gcf, '/home/ethan/fig_report/MI_and_coI_excluded_vs_included_triplets_GLV(p3).png');
