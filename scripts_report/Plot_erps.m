...
eeglab
[basefold, datatype, participantnum, ~, condition, participants, ~, re_epoch, dev_epochs, std_epochs, epoch_length, srate, low_cutoff, high_cutoff, baseline, start_cut_off, end_cut_off, kperm] = Get_param(0);
...
% [std, dvt] = impBB(participantnum, basefold, datatype, condition, srate, low_cutoff, high_cutoff,re_epoch, dev_epochs, std_epochs, epoch_length);

baseline = 101:151 %%change according to your baseline
...
%% PLOT ERPS
        ...
        for ch = 1 : dvt.nbchan
           nexttile(ch)
           stdshade_acj(squeeze(std_data_minus_mean(ch,101:451,:))',0.2,'r',dvt.times(101:451)); hold on
           stdshade_acj(squeeze(dvt_data_minus_mean(ch,101:451,:))',0.2,'g',dvt.times(101:451));
           hold off
        end
